(() => {
  ...
  function calc(t,paid){
    ...
    return {id:String(t.id||''),no:String(t.no||''),date:String(t.date||''),time:String(t.time||''),method:String(t.method||'Cash'),items:items.map(i=>({name:String(i.name||''),qty:Number(i.qty||0),price:Number(i.price||0)})),subtotal,discount,total,paid:Math.max(pay,total),change:Math.max(0,pay-total)};
  }

  function save(r){localStorage.setItem(LAST_RECEIPT_KEY,JSON.stringify(r));}
  function load(){try{return JSON.parse(localStorage.getItem(LAST_RECEIPT_KEY)||'null')}catch(_){return null}}

  function ensureStyles(){
    if(document.getElementById('receiptPrintStyles')) return;
    ...
    style.textContent=`
      ...
      .receipt-paper{width:80mm;max-width:100%;margin:0 auto;background:#fff;padding:10mm 5mm;color:#111;font:12px Arial,sans-serif;box-shadow:0 2px 12px rgba(0,0,0,.08)}
      ...
      @media print{body *{visibility:hidden!important}.receipt-overlay{position:static!important;display:block!important;background:#fff!important;padding:0!important}.receipt-dialog{width:auto!important;max-height:none!important;box-shadow:none!important;border:0!important}.receipt-dialog-head,.receipt-actions{display:none!important}.receipt-preview{padding:0!important;background:#fff!important}.receipt-paper{width:80mm!important;max-width:none!important;box-shadow:none!important;padding:3mm!important;margin:0!important}}
    `;
  ...
  function receiptHtml(r){
    ...
    const cash=r.method.toLowerCase()==='cash'?`<div class="line"><span>Bayar</span><b>${money(r.paid)}</b></div><div class="line"><span>Kembalian</span><b>${money(r.change)}</b></div>`:'';
  ...
  function closePreview(){const el=document.getElementById('receiptOverlay');if(el)el.remove()}
  function showPreview(r){
    ensureStyles(); closePreview();
    const overlay=document.createElement('div'); overlay.id='receiptOverlay'; overlay.className='receipt-overlay';
    overlay.innerHTML=`<div class="receipt-dialog"><div class="receipt-dialog-head"><span>Struk Transaksi</span><button id="receiptClose" style="border:0;background:transparent;font-size:20px;cursor:pointer">✕</button></div><div class="receipt-preview">${receiptHtml(r)}</div><div class="receipt-actions"><button id="receiptClose2">Tutup</button><button class="print-main" id="receiptPrint">🖨️ Cetak Struk</button></div></div>`;
  ...
  async function observeCheckoutResponse(){
    ...
    window.fetch=async function(input,init){
      ...
      let body=null;
      try{if(typeof reqBody==='string') body=JSON.parse(reqBody)}catch(_){ }
      ...
      if(body?.action==='checkout'){
        try{
          ...
          if(payload?.ok && payload?.data){
            ...
            const r=calc(transaction,paid);
            save(r);
            setTimeout(()=>showPreview(r),150);
          }
        }catch(_){ }
      }
      return response;
  ...
  function watchLocalCheckout(){
    const btn=document.getElementById('checkoutBtn');
    if(!btn || btn.dataset.receiptLocalWatch==='1') return;
    btn.dataset.receiptLocalWatch='1';
    btn.addEventListener('click',()=>{
      const before=load();
      const deadline=Date.now()+5000;
      const timer=setInterval(()=>{
        try{
          const raw=JSON.parse(localStorage.getItem('deanti_bakery_v2')||'{}');
          const txs=Array.isArray(raw.transactions)?raw.transactions:[];
          if(txs.length){
            const latest=txs[txs.length-1];
            if(!before || String(latest.id)!==String(before.id)){
              const paid=Number(document.getElementById('paidAmount')?.value||0);
              const r=calc(latest,paid); save(r); clearInterval(timer); showPreview(r); return;
            }
          }
        }catch(_){ }
  ...
  function ensureLastReceiptButton(){
    const payment=document.querySelector('.cart-panel .payment,.payment');
    if(!payment) return;
    let b=document.getElementById('printReceiptBtn');
    if(!b){b=document.createElement('button');b.id='printReceiptBtn';b.type='button';b.className='ghost full';b.style.marginTop='8px';payment.appendChild(b)}
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ensureStyles();
    await observeCheckoutResponse();
    watchLocalCheckout();
    ensureLastReceiptButton();
    setTimeout(()=>{watchLocalCheckout();ensureLastReceiptButton()},800);
  });
})();
