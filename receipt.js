(() => {
  ...
  function calculateReceipt(transaction, paid) {
    ...
    return {
      ...
      discount,
      total,
      paid: Math.max(total, paidValue),
      change: Math.max(0, paidValue - total)
    };
  }
  ...
  function printReceipt(receipt) {
    if (!receipt) {
      toast('Belum ada transaksi yang bisa dicetak');
      return;
    }

    const popup = window.open('', '_blank', 'width=420,height=720');
    if (!popup) {
      alert('Jendela cetak diblokir browser. Izinkan pop-up untuk aplikasi Deanti Bakery, lalu klik Cetak Struk lagi.');
      return;
    }
    ...
    const cashSection = receipt.method.toLowerCase() === 'cash'
      ? `
        <div class="line"><span>Bayar</span><b>${money(receipt.paid)}</b></div>
        <div class="line"><span>Kembalian</span><b>${money(receipt.change)}</b></div>
      `
      : '';
...
<title>${esc(receipt.no || 'Struk')} - Deanti Bakery</title>
<style>
...
h1 { margin: 0; text-align: center; font-size: 21px; }
.subtitle { text-align: center; font-size: 10px; margin: 3px 0 10px; }
.meta { border-top: 1px dashed #888; border-bottom: 1px dashed #888; padding: 7px 0; margin-bottom: 7px; }
.meta div, .line { display: flex; justify-content: space-between; gap: 8px; margin: 4px 0; }
...
.total { border-top: 1px solid #111; margin-top: 8px; padding-top: 7px; font-size: 15px; font-weight: 700; }
.footer { text-align: center; margin-top: 14px; font-size: 10px; }
.print-btn { width: 100%; border: 0; padding: 10px; margin-top: 14px; border-radius: 7px; background: #b36a2c; color: white; font-weight: 700; }
...
<div class="receipt">
  <h1>Deanti Bakery</h1>
  <div class="subtitle">Kasir & Pembukuan</div>
  <div class="meta">
    <div><span>No. Transaksi</span><b>${esc(receipt.no)}</b></div>
    <div><span>Tanggal</span><span>${esc(receipt.date)} ${esc(receipt.time)}</span></div>
    <div><span>Pembayaran</span><span>${esc(receipt.method)}</span></div>
  </div>
  <table>
    <thead><tr><th>Item</th><th class="center">Qty</th><th class="right">Jumlah</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>
  <div class="line"><span>Subtotal</span><b>${money(receipt.subtotal)}</b></div>
  <div class="line"><span>Diskon</span><b>${money(receipt.discount)}</b></div>
  <div class="line total"><span>Total</span><b>${money(receipt.total)}</b></div>
  ${cashSection}
...
<script>window.onload = () => setTimeout(() => window.print(), 300);<\/script>
  ...
  function ensureButton() {
    ...
    button.textContent = loadReceipt() ? '🖨️ Cetak Struk Terakhir' : '🖨️ Cetak Struk Terakhir';
  ...
  function watchCheckout() {
    ...
    checkoutButton.addEventListener('click', () => {
      ...
      let beforeCount = 0;
      try {
        beforeCount = Array.isArray(window.db?.transactions) ? window.db.transactions.length : 0;
      } catch (_) {}

      const started = Date.now();
      const timer = setInterval(() => {
        try {
          const transactions = Array.isArray(window.db?.transactions)
            ? window.db.transactions
            : [];

          if (transactions.length > beforeCount && transactions.length) {
  ...
  function showReceiptNotice(receipt) {
    ...
    btn.style.cssText = 'border:0;background:#b36a2c;color:#fff;padding:7px 11px;border-radius:8px;font-weight:700;cursor:pointer;';
    btn.onclick = () => printReceipt(receipt);
    box.appendChild(btn);
